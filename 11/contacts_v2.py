...
def custom_sort(contacts):
	...
	return merge( merge_sort(contacts[:middle]), merge_sort(contacts[middle:]) )
...
def wildcard_match(text, pattern):
    ...
    # '*' Wild card special cases
        ...
        if next_position >= 0: # If the letter after * is in text, recurse
            return match(pattern[1:], text[next_position:])
...
def ternary_search(array, query):
	...
	if array_size in [1,2]:
		if wildcard_match(array[0], query):
			return array[0]

		elif array_size == 2 and wildcard_match(array[1], query):
			return array[1]
	...
	if wildcard_match(array[cut_one], query):
		return array[cut_one]
	elif wildcard_match(array[cut_two], query):
...
def contact_output_format(contacts):
	...
	for contact in contacts:
		output += contact[:contact.find(",")] + "{:.<10}|"
...
def main():
	...
	while choice != "4":
		if choice == "1":
			...
			print(search_contacts(file_path, query))
